...
export function GetDummyRoute(game) {
  var route = new Route.Route(game, "Red Any% Glitchless - Exarion Route");
    var srNew = route.addNewSection("New Game");
      srNew.addNewDescription("Clear any existing save file by pressing Up + B + Select on the game title screen.");
      srNew.addNewDescription("New game: text speed fast, animations off, style shift.");
      srNew.addNewDescription("Start a new game, and name yourself and your rival a one character name.");
    var srPallet = route.addNewSection("Pallet Town", "It's time to get rolling!");
      srPallet.addNewDescription("Exit out of your home, and head north towards Route 1. Prof. Oak will stop you and lead you back to his lab. After he and your rival are done talking, select the middle Pokeball on the table to get Squirtle. Name it a one character name, and go to head out of the lab.");
      srPallet.addNewEntry("Get Pokemon", "#squirtle:5");
      srPallet.addNewDescription("Your rival will stop you for a battle.");
      srPallet.addNewEntry("Rival 1", "Tail Whip x1-2, then Tackle until it faints.");
      srPallet.addNewDescription("Head out of the lab, and north to Route 1");
    var srParcel = route.addNewSection("Getting the Parcel to Oak");
      var srR1 = srParcel.addNewSection("Route 1");
        ...
        srR1.addNewDescription("Head north through the route to Viridian City.");
      srParcel.addNewDescription("Head straight into the Mart and collect Oak's Parcel. Exit, and head south back to Route 1.");
      srParcel.addNewDescription("Head south, utilizing ledges to avoid as much grass as you can. If you have yet to kill an encounter, take the shorter route through the second-to-last grass patch; otherwise, walk around it.");
      srParcel.addNewDescription("Head to the lab, finish the lengthy conversation with Prof. Oak and head back north once more.");
    var srNidoran = route.addNewSection("Getting Nidoran!");
      srNidoran.addNewDescription("Head north to Viridian City again, remembering to kill an encounter if you haven't already.");
      var srViridian1 = srNidoran.addNewSection("Viridian City");
        srViridian1.addNewDescription("Head back into the Mart for the first shopping trip.");
        srViridian1.addNewEntry("Shop", "Buy 3-8 Poke Balls.");
        srViridian1.addNewDescription("Head out of the Mart, and west to Route 22.");
      var srR22 = srNidoran.addNewSection("Route 22");
        ...
        srR22.addNewDescription("Return to Viridian City and go north, picking up the tree potion, and entering Viridian Forest");
    var srForest = route.addNewSection("Viridian Forest");
      srForest.addNewDescription("Avoid encounters as much as possible by walking on encounterless grass tiles.");
