export const installRouter = (locationUpdatedCallback, getScroll, setScroll) => {
  document.body.addEventListener('click', e => {
    ...
    const anchor: HTMLAnchorElement = <HTMLAnchorElement>(e.composedPath().filter(n => (<any>n).tagName === 'A')[0]);
  ...
  document.body.addEventListener('navigate', (e: CustomEvent<{ href: string; external: boolean; }>) => {
